...
namespace PipeFlood {
  ...
  public:
    ...
    void key(sf::RenderWindow* window, sf::Event* event, PipeFlood::InputInfo* input) {
      ...
      map.selector.sprite.setPosition(sf::Vector2f(cursor.x * map.tileResized.x, cursor.y * map.tileResized.y));
    }
    ...
    void draw(sf::RenderWindow* window) {
      ...
      for (uint16_t x = 0; x < map.size.x; x++) {
        for (uint16_t y = 0; y < map.size.y; y++) {
         //auto sprite = map.tile(v2{ x, y });
          auto sprite = map.vWaterTiles[map.field[x][y]].sprite;
          window->draw(sprite);
        }
      }
      window->draw(map.selector.sprite);
    }
